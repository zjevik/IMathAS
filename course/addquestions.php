...
if (!(isset($teacherid))) { // loaded by a NON-teacher
	...
	*/
	...
	$placeinhead .= "<script type=\"text/javascript\" src=\"$imasroot/javascript/addqsort.js?v=101719\"></script>";
	...
	if ($sessiondata['selfrom'.$aid]=='lib') { //selecting from libraries
		...
		if (!$beentaken) {
			...
			if (isset($search) && ($searchall==0 || $searchlikes!='' || $searchmine==1)) {
				...
				$query .= " ORDER BY FIELD(imas_library_items.libid,$llist),imas_library_items.junkflag,imas_questionset.id";
				if ($searchall==1) {
					$query .= " LIMIT 300";
					$offset = 0;
...
echo '</div>';
if($displaymethod != "JustInTime"){
	require_once("addquestions-simpleUI.php");
