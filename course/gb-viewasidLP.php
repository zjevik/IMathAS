...
//(c) 2007 David Lippman
	...
	$bid = Sanitize::onlyInt($_GET['bid']);
	$backurl = "/assessment/showtest.php?cid={$cid}&id={$bid}";
	...
	if ($asid=="new" && $isteacher) {
		...
		header('Location: ' . $GLOBALS['basesiteurl'] ."/course/gb-viewasidLP.php?bid=$bid&stu=$stu&asid=$asid&from=$from&cid=$cid&uid=$get_uid");
	...
	if (isset($_REQUEST['clearattempt']) && $isteacher) {
		if (isset($_POST['clearattempt']) && $_POST['clearattempt']=='confirmed') {
			...
			if ($from=='isolate') {
				...
				header('Location: ' . $GLOBALS['basesiteurl'] . $backurl);
			}
			exit;
		} else {
			...
			$overwriteBody = true;
			$querystring = http_build_query(array('bid'=>$bid, 'stu'=>$stu, 'cid'=>$cid, 'asid'=>$asid, 'from'=>$from, 'uid'=>$get_uid));
			...
			$body .= '<form method="POST" action="gb-viewasidLP.php?'.$querystring.'">';
			$body .= '<p><button type=submit name="clearattempt" value="confirmed">'._('Really Clear').'</button> ';
			$body .= "<input type=button value=\"Nevermind\" class=\"secondarybtn\" onclick=\"window.location='gb-viewasidLP.php?$querystring'\"></p>\n";
			$body .= '</form>';
			//exit;
		}
	}
	if (isset($_REQUEST['clearreviewview']) && $isteacher) {
		...
		header('Location: ' . $GLOBALS['basesiteurl'] ."/course/gb-viewasidLP.php?stu=$stu&asid=$asid&from=$from&cid=$cid&uid=$get_uid");
	...
	if (isset($_REQUEST['clearq']) && $isteacher) {
		if (isset($_POST['clearq'])) { //postback
			...
			//echo "<p><input type=button onclick=\"window.location='gb-viewasidLP.php?stu=$stu&gbmode=$gbmode&cid=$cid&from=$from&asid={$_GET['asid']}&clearq={$_GET['clearq']}&uid={$get_uid}&confirmed=true'\" value=\"Really Clear\"> \n";
			//echo "<input type=button onclick=\"window.location='gb-viewasidLP.php?stu=$stu&gbmode=$gbmode&cid=$cid&from=$from&asid={$_GET['asid']}&clearq={$_GET['clearq']}&uid={$get_uid}&regen=1&confirmed=true'\" value=\"Really Clear and Regen\"> \n";
			//echo "<input type=button value=\"Nevermind\" class=\"secondarybtn\" onclick=\"window.location='gb-viewasidLP.php?stu=$stu&from=$from&gbmode=$gbmode&cid=$cid&asid={$_GET['asid']}&uid={$get_uid}'\"></p>\n";
			//exit;
		}
	}
	...
	if ($overwriteBody) { //doing a confirm action
		echo '<div><a href="'.Sanitize::encodeStringForDisplay('gb-viewasidLP.php?bid='.$bid.'&stu='.$stu.'&asid='.$asid.'&from='.$from.'&cid='.$cid.'&uid='.$get_uid.'&from='.$_GET['from']).'">';
	...
	if ($isteacher) {
		if ($line['agroupid']>0) {
			...
			echo "<a href=\"gb-viewasidLP.php?stu=$stu&cid=$cid&asid={$asid}&from=$from&uid=$get_uid&breakfromgroup=true\">Separate from Group</a></p>";
		}
	}
	if ($myrights == 100 && $line['lti_sourcedid']!='') {
		echo '<p class=small>LTI sourced_id: '.Sanitize::encodeStringForDisplay($line['lti_sourcedid']).'</p>';
	}	
	echo "<form id=\"mainform\" method=post action=\"gb-viewasidLP.php?bid=$bid&stu=$stu&cid=$cid&from=$from&asid={$asid}&update=true\">\n";
	if ($isteacher) {
		echo "<div class=\"cpmid\"><a href=\"gb-viewasidLP.php?bid=$bid&stu=$stu&cid=$cid&asid={$asid}&from=$from&uid=$get_uid&clearattempt=true\" onmouseover=\"tipshow(this,'Clear everything, resetting things like the student never started.  Student will get new versions of questions.')\" onmouseout=\"tipout()\">Clear Attempt</a> | ";
	...
	if (($line['timelimit']>0) && ($line['endtime'] - $line['starttime'] > $line['timelimit'])) {
		...
		if ($isteacher) {
			echo "<a href=\"gb-viewasidLP.php?stu=$stu&starttime=$reset&asid={$asid}&from=$from&cid=$cid&uid=$get_uid\">Clear overtime and accept grade</a></p>\n";
		}
	}
	...
	if (isset($_GET['lastver'])) {
		...
		$lastanswers = explode("~",$line['lastanswers']);
		echo "<p>";
		echo "<a href=\"gb-viewasidLP.php?stu=$stu&asid={$asid}&from=$from&cid=$cid&uid=$get_uid\">Show Scored Attempts</a> | ";
		echo "<b>Showing Last Attempts</b> | ";
		echo "<a href=\"gb-viewasidLP.php?stu=$stu&asid={$asid}&from=$from&cid=$cid&uid=$get_uid&reviewver=1\">Show Review Attempts</a>";
		echo "</p>";
		...
		echo "<p>";
		echo "<a href=\"gb-viewasidLP.php?stu=$stu&asid={$asid}&from=$from&cid=$cid&uid=$get_uid\">Show Scored Attempts</a> | ";
		echo "<a href=\"gb-viewasidLP.php?stu=$stu&asid={$asid}&from=$from&cid=$cid&uid=$get_uid&lastver=1\">Show Last Graded Attempts</a> | ";
	...
	for ($i=0; $i<count($questions);$i++) {
		...
		if ($isteacher || $istutor) {
			...
			if ($isteacher) {
				...
				echo " | <a href=\"gb-viewasidLP.php?stu=$stu&cid=$cid&from=$from&asid={$asid}&uid=$get_uid&clearq=$i\">Clear Score</a> ";
