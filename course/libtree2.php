...
END;
	...
	$query .= " ORDER BY imas_libraries.name ASC,imas_libraries.federationlevel DESC,imas_libraries.id";
	if (count($qarr)==0) {
		$stm = $DBH->query($query);
	} else {
	...
	if (isset($_GET['libtree']) && $_GET['libtree']=="popup") {
		...
		<input type=button value="Check all between" onClick="checklib(this.form)">
$colorcode
</form>
</body>
</html>
