...
function display($pic) {
	...
	$return .= "<script>
		...
		function loadHeatmap(element){
			var parentID = 'heatmap'+divID;
			$(element).parent()[0].id = parentID;
			var heatmapContainedQuery = '#'+parentID;
			$(heatmapContainedQuery+' canvas').remove();
			var heatmapInstance = h337.create({
				container: document.querySelector(heatmapContainedQuery),
				blur: 0.07,
				max: 100,
				min: 0,
				maxOpacity: .8,
			...
			heatmap_instance[divID] = heatmapInstance;
			canvasCopy = $('#'+parentID+' .heatmap-canvas').clone();
			canvasCopy[0].id = 'canvas'+divID++;
			$('#'+parentID+' .heatmap-canvas').parent().append(canvasCopy)
			
			// Code for placing marker to the canvas
			var canvas = document.getElementById(canvasCopy[0].id);
			...
			function drawAll(writeAnswer = true) {
			...
			if($('#headergb-viewasid').length > 0){
				//In Gradebook
				studansArr = $(element).closest('.question').find('input[type=text]').val().split(',');
				if(studansArr.length >= 2){
					// Convert from Permille
					studansArr[0] = studansArr[0]*Scanvas.width()/1000;
					studansArr[1] = studansArr[1]*Scanvas.width()/1000;
			...
			// Show answer code
			$(element).closest('.question').find('.sabtn').on('click',function(ev) {
				ev.stopImmediatePropagation();
				$('.question span[id*=ans]').hide();
				console.log('tesrt');
				data = JSON.parse($(ev.target).parent().find('span').text().trim());
				var local_heatmapInstance;
				if($('.sabtn').length == 1){
					//only one answer button
					local_heatmapInstance = heatmap_instance[0];
				} else{
					//many answer buttons - in gradebook
					local_heatmapInstance = heatmap_instance[parseInt($(this).attr('aria-controls').replace(/\D/g,''))];
				}
				data = convertHeatmapInstanceFromPermille(data,local_heatmapInstance._renderer._width);
				local_heatmapInstance.setData(data);
		...
		$('.heatmap img').each(function(){
			$(this).attr('src',$(this).attr('src'));
		});
		if(typeof heatmapLoaded === 'undefined'){
			divID = 0;
