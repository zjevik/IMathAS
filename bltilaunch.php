...
if (isset($_GET['launch'])) {
	...
	if ($stm->rowCount()==0) {$useragent=$_SERVER['HTTP_USER_AGENT'];
		$error = "No authorized session exists. This is most likely caused by your browser blocking third-party cookies.  Please adjust your browser settings and try again. If you are using Safari, you may need to disable Prevent Cross-Site Tracking.";
	...
	$_SESSION['lti_context_title'] = (!empty($_REQUEST['context_title']))?$_REQUEST['context_title']:$_REQUEST['context_id'];
	...
	if (isset($_SESSION['place_aid'])) {
		...
		if ($stm->rowCount()==0) {
			...
			if ($copycourse == "yes") {
				...
				$stm = $DBH->prepare($query);
				$stm->execute(array(':name'=>$sessiondata['lti_context_label']." - ".$sessiondata['lti_context_title'], ':ownerid'=>$userid, ':enrollkey'=>$randkey, ':hideicons'=>$hideicons, ':picicons'=>$picicons,
...
if (isset($_GET['launch'])) {
	...
	if ($stm->rowCount()==0) {
        ...
        { 
			...
			//echo "desktop";
			if($browser == 'safari'){
				echo "<br><img style='display: block; margin-left: auto; margin-right: auto;object-fit:contain; width:100%;' src='./img/safari_cookies.png'>";
			}
        }
		reporterror($error);
